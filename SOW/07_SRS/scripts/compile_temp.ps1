...
# Clear output file and add cover page
@"
...
function Append-Section {
    ...
    if (Test-Path $Path) {
        Add-Content -Path $OutputFile -Value $PageBreak
...
function Append-Folder {
    ...
    if (Test-Path $Folder) {
        Add-Content -Path $OutputFile -Value $PageBreak
        Add-Content -Path $OutputFile -Value "# $SectionName`n"
...
function Append-Module {
    ...
    if (Test-Path $ModuleFolder) {
        Add-Content -Path $OutputFile -Value $PageBreak
        Add-Content -Path $OutputFile -Value "# $ModuleName`n"
...
# Footer
Add-Content -Path $OutputFile -Value "`n`n---`n`n**End of Document - NewPOPSys SRS v1.38**"

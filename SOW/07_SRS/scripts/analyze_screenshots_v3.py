...
SCREEN_MAPPINGS = {
    ...
    'M004': ['install_photos', 'mobile_install_photos', 'mobile_install_slots'],
...
def analyze_screenshots(srs_root, screenshots_root):
    ...
    for screen_id, patterns in sorted(SCREEN_MAPPINGS.items()):
        ...
        # Use simple text instead of emoji to avoid encoding issues if system doesn't like it
        status_text = "[ OK ]" if found_match else "[MISSING]"
        match_name = found_match if found_match else "-"
        
        print(f"| {screen_id} | {', '.join(patterns)} | {status_text} | {match_name} |")

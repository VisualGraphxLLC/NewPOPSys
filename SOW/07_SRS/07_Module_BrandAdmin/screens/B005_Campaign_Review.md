...
```mermaid
stateDiagram-v2
    DRAFT --> SCHEDULED: publish (start > today)
    DRAFT --> PUBLISHED: publish (start <= today)
    SCHEDULED --> PUBLISHED: start_date
```
...
```mermaid
flowchart TD
    S1[Step 1: Stores] -->|back| S2[Step 2: Kit]
    S2 -->|continue| S3[Step 3: Review]
    S3 --> Save[Save Draft]
    S3 --> Publish[Publish]
    S3 --> Back[Back to Kit]
    Save --> List[Campaign List]
    Publish --> Detail[Campaign Detail]
```
